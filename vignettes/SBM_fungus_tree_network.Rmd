...
Instead of considering the binay network tree-tree we may consider the weighted network where the linik between two trees is the number of fungi they share. 
...
One can also plot the expected network which, in case of the Poisson model, corresponds to the expection  of connection between any pair of nodes in the network.
...
sbm.cov <- estimateSimpleSBM(netMat = as.matrix(tree_tree),model = 'poisson',directed =FALSE, covariates  = fungus_tree_network$covar_tree,estimOptions = list(verbosity = 0))
```
...
We now analyse the bipartite tree/fungi interactions. The incidence matrix can be plotted with the function \cote{plotMyMatrix}
...
bipartite.sbm <- estimateBipartiteSBM(netMat = as.matrix(fungus_tree_network$fungus_tree),model = 'bernoulli',estimOptions = list(verbosity = 0))
```

```{r bipartite.sbm fields}
bipartite.sbm$nbNodes
bipartite.sbm$nbBlocks
bipartite.sbm$connectParam
```

We can now plot the reorganized matrix. 

```{r plot bipartite}
plot(bipartite.sbm, rowLabel = 'fungis',colLabel = 'tree')
```
