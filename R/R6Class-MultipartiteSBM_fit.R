    ...
    public = list(
      ...
      initialize = function(netList) {
        ...
        arch <- map_if(netList, ~ inherits(.x, "SimpleSBM_fit"),
          ...
          .else = function(net) setNames(unname(net$dimLabels), c("from", "to"))
        ) %>%
          bind_rows() %>%
          map(factor, levels = dimLabels) %>%
          map_df(as.numeric) %>%
          as.matrix()
        super$initialize(
          model = map_chr(netList, "modelName"),
          architecture = arch,
          directed = directed,
          nbNodes = nbNodes,
          dimLabels = dimLabels,
      ...
      optimize = function(estimOptions) {
        ...
        givenclassif <- currentOptions$givenclassif
